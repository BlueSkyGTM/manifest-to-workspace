...
to hand off between sessions during long runs (the curriculum pass spans sessions).
...
memory-manager records it. RULE: it writes to the readable learned-seam files and the logs, which
are THEN indexed into GBrain for retrieval. File is canonical; GBrain is the projection. Never write
a seam decision only into GBrain — the human must be able to read and grade it.
...
The curriculum pass IS the seam layer's training run. Every seam call the human makes is training
...
style preference; it is what makes the dry run gradeable.
